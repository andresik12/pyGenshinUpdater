...
# optimization audio list for prevent checking checksum of files, that not be modified
...
def audio_file_finder(hdiff_audio: list, audio_checksum: list):
	...
	for hdiff_file in hdiff_audio:
		...
		for checksum in audio_checksum:
			...
			#TODO What happened if audio pck file wouldn't exist, but hdiff will be?
	return audio_list

# Opening checksum file and returning list of dict
# filled by file name and md5 checksum
def reading_and_formatting_checksum_file(checksum_file_path: Path):
...
# Returning list, that contains filtered checksum list by suffix
...
def files_checksum_checker(checksum_to_compare: str, file_path: Path):
	...
	if calculated_md5 == checksum_to_compare:
		...
		print('Something wrong with', file_info[1], 'at', file_info[0], sep=" ")
		return False
...
def audio_files_checksum_processing(hdiff_files_list: list, checksum_list: list, path_audio: Path):
	...
	for file in files_list_for_patching:
		...
		if not files_checksum_checker(file['md5'], Path.joinpath(path_audio, file['FileName'])):
			...
			answer = input('Continue? Enter "no" if you want to abort: ') or "yes"
			if answer[0].lower() == 'n':
				sys.exit(1)
...
def audio_assets_patching(original_files_dir: Path, patch_files_dir: Path, patch_dir: Path, file_list: list, checksum_list: list[dict]):
	...
	hpatch_exec = Path.joinpath(Path(TOOLS_DIR).resolve(), "hpatchz" + EXEC_EXTENSION) #add executable extension for NT
	...
	for file_name in file_list:
		...
		subprocess.call([hpatch_exec, Path.joinpath(original_files_dir, file_name_stem), Path.joinpath(patch_dir_abs, file_name), path_of_patched_pck], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
		#check for hpatch create patched pck
		if path_of_patched_pck.exists():
			print(str(file_name_stem), "patched, checking checksum..", sep=" ")
			#checking checksum before replacing OG file
			if files_checksum_checker(checksum_pck['md5'], path_of_patched_pck):
				print("Replacing old pck by patched pck..")
...
def audio_localization_package_updater(path_game_root: Path, path_update_root: Path):
	...
	checksum_list_original = reading_and_formatting_checksum_file(Path.joinpath(path_game_root, checksum_file_name))
	checksum_list_patched = reading_and_formatting_checksum_file(Path.joinpath(path_update_root, checksum_file_name))
...
def game_audio_updater(path_game_root: Path, path_update_root: Path):
	...
	checksum_list_original = checksum_files_filter(reading_and_formatting_checksum_file(path_checksum_file_current), ".pck")
	checksum_list_patched = checksum_files_filter(reading_and_formatting_checksum_file(path_checksum_file_new), ".pck")
...
def dir_Selector(case_type: int):
	...
	match case_type:
		case 0:
			if len(DIR_GAME_FILES_UPDATE) < 1:
...
def main():
	...
	print("For entering path of directories, you can simply drag and drop folder at cmd window (for windows), terminal window (for macos), konsole (for linux)")
